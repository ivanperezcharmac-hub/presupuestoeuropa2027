  ...
  return (
    <div className="py-2 border-b last:border-0" style={{ borderColor: 'var(--border)' }}>
      <input type="text" placeholder="Artículo..." value={item.name || ''} onChange={e => update('name', e.target.value)}
        className="w-full px-2 py-1 rounded-lg text-sm mb-1.5 outline-none" style={{ border: '1.5px solid var(--border)', background: 'var(--cream)' }} />
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          <input type="number" placeholder="0" min="0" value={item.cost || ''} onChange={e => update('cost', e.target.value)}
            className="w-full pl-5 pr-2 py-1 rounded-lg text-sm font-mono-dm outline-none" style={{ border: '1.5px solid var(--border)', background: 'var(--cream)' }} />
        </div>
        <input type="text" placeholder="Ciudad..." value={item.nota || ''} onChange={e => update('nota', e.target.value)}
          className="flex-1 px-2 py-1 rounded-lg text-sm outline-none" style={{ border: '1.5px solid var(--border)', background: 'var(--cream)' }} />
        <button onClick={remove} className="text-xs px-2 rounded-lg transition-colors hover:bg-red-50" style={{ color: 'var(--lite)' }}>✕</button>
      </div>
    ...
    <div className="flex-1 min-w-0">
      <div className="flex justify-between items-center mb-3 pb-2" style={{ borderBottom: '2px solid var(--border)' }}>
        ...
        <div className="text-right">
          ...
          <div className="font-mono-dm text-sm font-semibold" style={{ color: 'var(--navy)' }}>{f$(totalEur * rate)}</div>
        </div>
      </div>
      ...
      <button onClick={addRow} className="w-full mt-2 py-1.5 rounded-lg text-xs font-medium transition-all hover:brightness-95"
        style={{ border: '1px solid var(--border)', color: 'var(--muted)', background: 'var(--cream)' }}>
        + Agregar
      </button>
    </div>
  );
  ...
  return (
    <div>
      ...
      {COMPRAS_CATS.map(cat => (
        <div key={cat.id} className="rounded-2xl p-4 mb-4" style={{ background: '#fff', border: '1px solid var(--border)', boxShadow: 'var(--sh)' }}>
          <div className="flex justify-between items-center mb-3 pb-3" style={{ borderBottom: '1px solid var(--border)' }}>
            ...
            <div className="text-right">
              ...
              <div className="font-mono-dm text-base font-semibold" style={{ color: 'var(--navy)' }}>
      ...
      <div className="rounded-2xl p-4" style={{ background: '#fff', border: '1px solid var(--border)' }}>
        <div className="flex justify-between items-center">
          ...
          <div className="text-right">
            <div className="font-mono-dm text-xs" style={{ color: 'var(--muted)' }}>€{Math.round(totalEur)}</div>
            <div className="font-mono-dm text-lg font-semibold" style={{ color: 'var(--navy)' }}>{f$(totalEur * eurUsd)}</div>
          </div>
        </div>
      </div>
    </div>
