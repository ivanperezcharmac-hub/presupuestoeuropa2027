...
export default function Alertas() {
  ...
  if (min && eurUsd < min) { alertMsg = `✅ EUR/USD en ${eurUsd.toFixed(4)} — ¡Por debajo del mínimo! Buen momento para comprar euros.`; alertColor = 'var(--grn-bg)'; }
  else if (max && eurUsd > max) { alertMsg = `⚠️ EUR/USD en ${eurUsd.toFixed(4)} — Por encima del máximo. El euro está caro.`; alertColor = 'var(--red-bg)'; }
  else if (min || max) { alertMsg = `📊 EUR/USD en ${eurUsd.toFixed(4)} — Dentro del rango configurado.`; alertColor = 'var(--surface2)'; }
  ...
  return (
    <div>
      ...
      <div className="rounded-2xl p-4 mb-4" style={{ background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--sh)' }}>
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          {[
            ...
            <div key={item.label} className="rounded-xl p-3 text-center" style={{ background: 'var(--surface2)', border: '1px solid var(--border)' }}>
              ...
              <input type={item.readOnly ? 'text' : 'number'} readOnly={item.readOnly} step="0.01" min="0.5" max="3"
                ...
                style={{ color: item.field === 'min' ? 'var(--grn)' : item.field === 'max' ? 'var(--red)' : 'var(--txt)', border: 'none' }} />
            </div>
          ))}
        </div>
        {alertMsg && (
          <div className="px-4 py-3 rounded-xl text-sm font-medium" style={{ background: alertColor, color: 'var(--txt)' }}>{alertMsg}</div>
        )}
      </div>
      <div className="rounded-2xl p-4" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {scenarios.map(sc => {
            ...
            return (
              <div key={sc.l} className="rounded-xl p-3 text-center" style={{ background: 'var(--surface2)', border: '1px solid var(--border)' }}>
                <div className="text-xs mb-1" style={{ color: 'var(--muted)' }}>{sc.l}</div>
                <div className="font-mono-dm text-base font-semibold"
                  style={{ color: isNow ? 'var(--txt)' : diff > 0 ? 'var(--red)' : 'var(--grn)' }}>
